...
class DiscBitBlockIndex {
 public:
  ...
  int64_t Lookup(const Slice& key) const;

  int64_t FinishSeek(const Slice& key, const Slice& probe_key,
                int64_t probe_pos, uint64_t pkey, int cmp) const;

  int64_t PkeyLookup(uint64_t pkey) const;
 ...
 private:
  ...
  int64_t PkeyLCP(const Slice& target, const Slice& key) const;
};
